...
MapAsTimeSeriesSet TDMap::getcumulative(string dir)
{
    ...
    if (dir=="x")
    {
        for (int j=0; j<val.size(); j++)
        {
            ...
            CTimeSeries<double> X;
            X.append(0,0);
            for (int i=0; i<val[j].size(); i++)
            {
                X.append(dx*i,val[j][i]*dx+X.GetC(X.n-1));
            }
            X = X/X.GetC(X.n-1);
    ...
    if (dir=="sym")
    {
        ...
        CTimeSeries<double> X0;
        X0.structured=true;
        X0.append(0,0);
        for (int i=0; i<val[0].size(); i++)
        {
            X0.append(-dx*i-dx,0.5*(val[i][0]+val[0][i])*dx+X0.GetC(X0.n-1));
        }
        X0 = X0/X0.GetC(X0.n-1);
        out.append(X0,-dy/2);
        ...
        for (int j=0; j<val.size(); j++)
        {
            ...
            for (int i=0; i<val[j].size(); i++)
            {
                X.append(dx*i+dx,0.5*(val[j][i]+val[i][j])*dx+X.GetC(X.n-1));
            }
            X = X/X.GetC(X.n-1);
            out.append(X,dy*j+dy/2);
        }
        CTimeSeries<double> Xn;
        Xn.structured=true;
        Xn.append(0,0);
        for (int i=0; i<val[val.size()-1].size(); i++)
        {
            Xn.append(up_lim_x + dx*i + dx/2,0.5*(val[val.size()-1][i]+val[i][val.size()-1])*dx+Xn.GetC(Xn.n-1));
        }
        Xn = Xn/Xn.GetC(Xn.n-1);
...
bool TDMap::readfromfile(const string &filename)
{
    ...
    vector<double> x = aquiutils::ATOF(aquiutils::getline(file,','));
    reset(x.size(),x.size(),0,1,0,1);
    for (int i=0; i<x.size(); i++)
    {
        vector<double> v = aquiutils::ATOF(aquiutils::getline(file,','));
...
{
    if (pngfilename=="")
        pngfilename = aquiutils::split(filename,'.')[0] + ".png";
    ofstream file(filename);
    if (logscale)
    {
